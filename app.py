...
if any(c.isspace() for c in API_KEY):
    raise RuntimeError("OPENAI_API_KEY contains whitespace. Paste it again without spaces/newlines.")
...
def polish():
    ...
    try:
        ...
        r = requests.post(f"{API_BASE}/chat/completions",
                          headers=_headers(), json=payload, timeout=60, proxies=_no_proxy())
        if not r.ok:
